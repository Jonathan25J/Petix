...
module.exports.run = async (client, message, args, ops, playlist) => {
  ...
  if (!data.connection)
    data.connection = await message.member.voice.channel.join().catch((err) => {
  ...
  if (!data.dispatcher) {
    ...
    var queueEmbed = new discord.MessageEmbed()
      .setDescription(
        `Added **${info.videoDetails.title}** to the queue [${message.author}]`
      )
      .setColor(message.guild.me.displayHexColor);
...
async function Play(client, ops, data, message) {
  var playEmbed = new discord.MessageEmbed()
    .setDescription(
      `Now playing **${data.queue[0].songTitle}** [${data.queue[0].requester}]`
    )
...
function Finish(client, ops, dispatcher, message) {
  ...
  if (fetchedData.queue.length > 0) {
    ...
    var voiceChannel = client.guilds.cache.get(dispatcher.guildID).me.voice
      .channel;
    ...
    if (voiceChannel) {
      var leaveEmbed = new discord.MessageEmbed()
        ...
        .setColor(
          client.guilds.cache.get(dispatcher.guildID).me.displayHexColor
        );
