...
module.exports.run = async (bot, message, args, client) => {
  ...
  if (!args[0]) {
    var confEmbed = new discord.MessageEmbed()
      ...
      .setThumbnail(
        "https://cdn.icon-icons.com/icons2/1141/PNG/512/1486395874-settings_80622.png"
      );
  ...
  if (args[0].includes("welcome")) {
    if ((args[0].includes("welcome"), !args[1]))
      return message.channel.send("Use set or remove");

    if (args[1].includes("set")) {
      if ((args[1].includes("set"), !args[2]))
        return message.channel.send("Set channel or message");
      ...
      if (args[2].includes("channel")) {
        ...
        message.channel.send(
          `You have changed your welcome channel to "${channelW}"`
        );
      }

      if (args[2].includes("message")) {
        if ((args[2].includes("message"), !args[3]))
          return message.channel.send("You didn't give any text");
        ...
        message.channel.send(
          `You have changed your welcome message to "${messageW}"`
        );
      }
    }
  ...
  if (args[0].includes("tickets")) {
    ...
    if (args[1].includes("set")) {
      ...
      if (args[2].includes("category")) {
        ...
        message.channel.send(
          `Tickets are set in the "${categoryName}" category`
        );
      }
    }
