...
module.exports.run = async (bot, message, args) => {
    ...
    var array = message.channel.rolePermissions(message.guild.roles.everyone).toArray()
    ...
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send(nPermission);
    ...
    if (lockdown === true) {
        message.channel.updateOverwrite(message.guild.roles.everyone, { SEND_MESSAGES: true }).then(() => {
            ...
            var lREmbed = new discord.MessageEmbed()
                .setColor(message.guild.me.displayHexColor)
                .setDescription(`✔ The channel ${message.channel} is out of a lockdown!`)
